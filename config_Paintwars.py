...
import robot_challenger_main
import robot_challenger_imene
import BOSY_robot_challenger
import pasali_uzun_5_robot_challenger
import BR_KA_challenger
import HR_HV_challenger
import equipe6_challenger
import zelen_challenger
import robot_challenger
import robot_challenger7
import robot_champion
import robot_braitenberg_chaser
...
def initialize_robots(arena_size=-1, particle_box=-1): # particle_box: size of the robot enclosed in a square
    ...
    robots = []
    for i in range(4):
        robots.append(robot_challenger_main.Robot_player(x_init_pos[0], arena_size//2-16+i*8, orientation_champion, name="", team="A"))
    for i in range(4):
        robots.append(equipe6_challenger.Robot_player(x_init_pos[1], arena_size//2-16+i*8, orientation_challenger, name="", team="B"))
    return robots
